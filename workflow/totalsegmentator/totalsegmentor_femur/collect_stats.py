...
def get_subject_femur_stats(subject_seg_dir, femur_filenames):
    ...
    for sample_femur_path in full_paths:
        ...
        sample_femur = get_largest_connected_component(
            sample_femur
        )  # some segmentations have islands of spurious segmentations
...
def save_total_voxel_stats_for_whole_dataset(
    subjects_path, femur_filenames, stats_out_path
):
    femur_meta_dict = {}
    header = ["left", "right"]
...
def analyze_stats(metadata_path):
    ...
    print(
        f"Total rows {len(df)} Empty rows",
        len(df[(df["left"] == 0) & (df["right"] == 0)]),
    )

    full_femur_df = df[
        df.transpose().all()
    ].copy()  # find ct scans with both left and right femur annotated
    full_femur_df.rename(columns={"Unnamed: 0": "subject_id"}, inplace=True)
    # median volume of the left and right femurs
    print(
        f'Median right {full_femur_df["right"].median()} left {full_femur_df["left"].median()}'
    )

    # left_p = full_femur_df['left'].quantile(0.4)
    threshold_50k = 50000
    full_femur_left_nonpartial = full_femur_df[(full_femur_df["left"] > threshold_50k)]
    full_femur_right_nonpartial = full_femur_df[
        (full_femur_df["right"] > threshold_50k)
    ]
    print(
        f"left {len(full_femur_left_nonpartial)} right {len(full_femur_right_nonpartial)}"
    )

    # save subjects with both left and right femur annotated
    full_femur_df.to_csv(
        Path(__file__).with_name("subjects_with_both_femur.csv"),
        columns=["subject_id"],
        index=False,
        header=False,
    )
    ...
    full_femur_left_nonpartial.to_csv(
        Path(__file__).with_name("subjects_with_left_femur.csv"),
        columns=["subject_id"],
        index=False,
        header=False,
    )

    full_femur_right_nonpartial.to_csv(
        Path(__file__).with_name("subjects_with_right_femur.csv"),
        columns=["subject_id"],
        index=False,
        header=False,
    )
...
if __name__ == "__main__":
    ...
    if OBTAIN_STATS:
        ...
        save_total_voxel_stats_for_whole_dataset(
            subjects_path, femur_filenames, stats_out_path
        )
