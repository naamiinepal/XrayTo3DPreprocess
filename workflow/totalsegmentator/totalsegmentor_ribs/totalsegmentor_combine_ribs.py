...
def get_totasegmentor_ribs_filenames() -> List[str]:
    ...
    for rib_num in rib_number:
        for pos in position:
            ribs_filenames.append(
                ribs_filename_template.format(position=pos, rib_number=rib_num)
            )
...
def process_totalsegmentor_subject(ct_path, seg_dir, ribs_filenames):
    ...
    rib_complete_seg_sitk = combine_segmentations(
        rib_seg_sitk, rib_seg_sitk[0], fill_label=1
    )

    stats_obj = get_segmentation_stats(rib_complete_seg_sitk)

    total_voxels = np.sum(
        [stats_obj.GetNumberOfPixels(l) for l in stats_obj.GetLabels()], dtype=np.int32
    )
