...
from xrayto3d_preprocess import call_rest_api, download_synapse, download_wget, getImage_TCIA_restAPI_URL, getImageMetaData_TCIA_restAPI_URL, zipDICOMtoNifti
...
CLINIC_RAW_PATH = 'external/XrayTo3DPreprocess/workflow/ctpelvic1k/download_links/clinic_raw.csv'
KITS_RAW_PATH = 'external/XrayTo3DPreprocess/workflow/ctpelvic1k/download_links/kits_raw.csv'
COLONOG_RAW_PATH = 'external/XrayTo3DPreprocess/workflow/ctpelvic1k/CTColonography_MetaData.csv'
...
def get_series_ID(patient_id: str):
    ...
    subdf = subdf[~subdf['Series Description'].str.contains(
        "Topo|topo")]['Series UID']  # avoid topograms
...
def download_segmentations():
    ...
    for dataset_name, seg_url, md5, zip_filename in df:
        ...
        extractall(str(download_file_path/zip_filename),
                   output_dir=zip_extract_dir)
...
def download_clinic_raw():
    """download images from CLINIC and CLINIC_METAL dataset. extract the zip into each subdirectory of the corresponding ct scan dir. MD5 hash for zip file available.

    known issues: if the raw zip has already been downloaded and extracted, this code will try and verify the zip file twice. Verifying can take >1 mins for large files. 
    """
...
def download_colonog_raw():
    ...
    for index, row in tqdm(df.iterrows(),total=len(df)):
        ...
        for sid in get_series_ID(patient_id):
            ...
            series_number = int(float(get_value_from_tcia_json_metadata(metadata_json,'Series Number')))
            if series_number == get_segmentation_series_number(segmentation_filename):
                with tempfile.TemporaryDirectory() as defaultTempDir:
                    ...
                    zipDICOMtoNifti(dicom_filepath, output_dir=str(RAW_OUT_DIR))
