...
json_path = "test_data/sub-verse004/sub-verse004_seg-subreg_ctd.json"
seg_path = "test_data/sub-verse004/seg.nii.gz"
...
for vb_id, *ctd in centroids:
    ...
    squared_distances = [
        (x - flipped_coords[0]) ** 2
        + (y - flipped_coords[1]) ** 2
        + (z - flipped_coords[2]) ** 2
        for x, y, z in zip(*seg_label_indices)
    ]
    ...
    vertebra_colormap[
        (
            np.where(
                (squared_distances > edges[infls][1])
                & (squared_distances < edges[infls][2])
            )
        )
    ] = 0.0
    ...
    plt.legend()
    plt.savefig("misc/vertebra_morphometry_1.png")
    plt.close()
    ...
    plt.savefig("misc/vertebra_morphometry_inflection_point.png")
    plt.close()

    fig = plt.figure()
    ax = plt.axes(projection="3d")
    ax.scatter3D(*seg_label_indices, s=2, c=squared_distances, alpha=0.2)
    plt.savefig("misc/vertebra_morphometry_3d.png")
    plt.close()

    # fig = plt.figure()
    # ax = plt.axes(projection="3d")
    # ax.scatter3D(
    #     *seg_label_indices, s=2, c=vertebra_colormap, cmap="viridis", alpha=0.2
    # )
